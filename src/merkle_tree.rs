...
impl MerkleTree {
    ...
    pub fn verify(&self, proofs: Vec<String>, mut leaf_index: usize) -> Option<bool> {
        ...
        for proof in proofs {
            if leaf_index % 2 == 1 {
                ...
                hash_for_verification = digest(proof + &hash_for_verification);
    ...
    pub fn add_node(&mut self, data: &str) -> Option<()> {
        if !is_power_of_2(self.og_data_vector_size) {
            ...
            new_data = digest(current_tree.first()?.to_owned() + current_tree.get(1)?); //calculate new root
...
fn extend_to_power2_size<T: Clone>(vec: Vec<T>) -> Vec<T> {
...
mod tests {
    ...
    fn verify_incorrect_proof_with_noise_string_tree() {
        ...
        let hash34 = digest(hash3 + &hash4) + "NOISE";
        let proof: Vec<String> = vec![hash1, hash34];
        assert!(!merkle_tree.verify(proof, 4).unwrap());
    }
